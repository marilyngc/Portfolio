                        ...
                        team.map((item, idx) => (
                            <div key={idx} className="flex flex-col sm:flex-row items-center   gap-11">
                                <div className="w-full md:w-1/3  ">
                                    <div className= {`relative w-44 h-44 border-8 shadow-lg rounded-full overflow-hidden bg-indigo-100 ${isDarkTheme ? ` dark:border-gray-900/80  `: `border-white  `} `}>
                                        <img
                                            src={item.avatar}
                                ...
                                <div className = "flex flex-col items-center sm:items-start gap-2 ">
                                    <h4 className={`text-2xl sm:text-4xl text-center sm:text-left font-bold  ${isDarkTheme ? `dark:text-slate-100 ` : `text-slate-600`}`}>{item.name}</h4>
                                    <p className={` text-sm sm:text-lg  max-w-xl rounded-2xl ${isDarkTheme ? `dark:text-slate-300` : `text-slate-700`}` }>{item.title}</p>
                                    
                                    <div className="mt-1 flex gap-4 text-gray-400">
                                    <div className="relative group/tooltip">
                                        <div className={`p-2.5 rounded-xl grid place-items-center  ${isDarkTheme ? `dark:bg-emerald-500 ` : `bg-emerald-300`}`}>
                                        <GiBullseye  className={` ${isDarkTheme ? `dark:text-white`: `text-emerald-700 `}`} />
                                        </div>
                                    ...
                                    <button  className={`flex items-center text-sm gap-3   py-2 px-4 rounded-lg font-semibold ${isDarkTheme ? `dark:text-indigo-100 dark:bg-indigo-500 hover:dark:bg-opacity-90 ` : `text-indigo-500 bg-indigo-100 hover:bg-opacity-80 `}`}>
